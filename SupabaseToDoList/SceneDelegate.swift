...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        Task {
            do {
                let currentUser = try await SupabaseManager.shared.client.auth.session
                
                // currentUser varsa TabBar'a git
                await MainActor.run {
                ...
                await MainActor.run {
                    let board = UIStoryboard(name: "Main", bundle: nil)
                    let loginVC = board.instantiateViewController(identifier: "ViewController")
    ...
    @objc func handleLogout() {
        let board = UIStoryboard(name: "Main", bundle: nil)
        let loginVC = board.instantiateViewController(identifier: "ViewController") // Login ekranının identifier'ı
        window?.rootViewController = loginVC
    }
